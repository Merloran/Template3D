...
Handle<DisplayManager::Window> DisplayManager::create_custom_window(const String& name, const IVector2& size, const DynamicArray<Hint>& hints)
{
	const auto iterator = windowsNameMap.find(name);
	if (iterator != windowsNameMap.end())
	{
	...
	windowsNameMap[name] = handle;

	return handle;
}

Handle<DisplayManager::Window> DisplayManager::create_preset_window(const String& name, const IVector2& size, EWindowPreset preset)
{
...
DisplayManager::Window& DisplayManager::get_window(const String& name)
{
	const auto& iterator = windowsNameMap.find(name);
	if (iterator == windowsNameMap.end() || iterator->second.id >= windows.size())
	{
...
DisplayManager::Window& DisplayManager::get_window(const Handle<Window> handle)
{
	if (handle.id >= windows.size())
	{
...
Handle<DisplayManager::Window> DisplayManager::get_window_handle(const String& name) const
{
	const auto& iterator = windowsNameMap.find(name);
	if (iterator == windowsNameMap.end())
	{
...
DisplayManager::Window& DisplayManager::get_current_window()
{
...
Void DisplayManager::swap_buffers()
{
	...
	glfwSwapBuffers(get_current_window());
}
